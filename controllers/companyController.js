    ...
    for (const article of articles) {
      ...
      if (extraSectionContent) {
        const clientIntroduction =
          await extractClientIntroduction(extraSectionContent);
        const clientSelection =
          await extractClientSolution(extraSectionContent);
        ...
        // What did you find most impressive or unique about this company?
        articleData.impressiveness =
          await extractClientImpressions(extraSectionContent);

        // Are there any areas for improvement or something they could have done differently?
        articleData.improvement =
          await extractClientImprovement(extraSectionContent);
...
const handleCheckPaginationReview = async (page) => {
  const ulElement = await page.$(
    "nav.profile-reviews--pagination ul.sg-pagination",
  );
...
const extractClientIntroduction = async (extraSectionContent) => {
  try {
    return await extraSectionContent.$eval(
      'div[data-link-text="Background"]',
      (ele) => {
        const paragraphs = Array.from(
          ele.querySelectorAll(
            ":scope > *:not(h5.profile-review__extra-title)",
          ),
        );
...
const extractClientImpressions = async (
  extraSectionContent,
  question = IMPRESSIONS_QUESTION,
) => {
  return await extraSectionContent.$eval(
    'div[data-link-text="Results"]',
    (ele, question) => {
      const paragraphs = Array.from(
...
const extractClientImprovement = async (extraSectionContent) => {
  return await extractClientImpressions(
    extraSectionContent,
  ...
  try {
    return await extraSectionContent.$eval(
      ...
      (ele, result) => {
        ...
        const firstQuestion = paragraphs.findIndex(
          (p) =>
            p
              .querySelector("strong")
              ?.textContent?.includes("How did you get") ||
            p
              .querySelector("strong")
              ?.textContent?.includes("How did you come") ||
            p
              .querySelector("strong")
              ?.textContent?.includes("How did you find"),
        );

        if (firstQuestion !== -1 && firstQuestion + 1 < paragraphs.length) {
          if (paragraphs[firstQuestion + 1].tagName === "UL") {
            const liElements =
              paragraphs[firstQuestion + 1].querySelectorAll("li");
            const contentArray = Array.from(liElements).map((liElement) => {
              let liText = liElement.textContent.trim();
              // If there isn't a period at the end of the string, add one.
              if (!liText.endsWith(".")) {
            ...
            result.selectionProcess =
              paragraphs[firstQuestion + 1].textContent.trim();
          }
        }
        ...
        if (secondQuestion !== -1 && secondQuestion + 1 < paragraphs.length) {
          if (paragraphs[secondQuestion + 1].tagName === "UL") {
            ...
            result.selectionCriteria =
              paragraphs[secondQuestion + 1].textContent.trim();
          }
        }
